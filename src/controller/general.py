...
async def send_message_by_id(connector: SCSCBotConnector, body: dict):
    if body.get("embed"):
        embed = discord.Embed(
            title=body["embed"].get("title"),
            description=body["embed"].get("description"),
            color=discord.Color(body["embed"].get("color")),
        )
...
async def get_id_from_name(connector: SCSCBotConnector, body: dict):
    channel = connector.get_channel(connector.slugify(body["channel_name"]))
    if channel:
