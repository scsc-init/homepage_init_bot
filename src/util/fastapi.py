    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            res = await client.get(
                f"http://{get_settings().main_backend_host}:8080/api/user/profile",
                headers={"x-api-secret": get_settings().api_secret, "x-jwt": jwt},
            )
    except Exception:
...
async def login():
    if await get_logged_in():
        return
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            res = await client.post(
                f"http://{get_settings().main_backend_host}:8080/api/user/login",
                headers={"x-api-secret": get_settings().api_secret},
                json={
                    "email": "bot@discord.com",
                    "hashToken": generate_user_hash("bot@discord.com"),
                },
            )
    except Exception:
        logger.error(
            "err_type=login ; err_code=504 ; Login request failed", exc_info=True
        )
    ...
    if res.status_code == 200:
        ...
        if not token:
            logger.error(
                "err_type=login ; err_code=502 ; Login succeeded but no token was returned"
            )
            raise HTTPException(
                status_code=502, detail="Login succeeded but no token was returned"
            )
